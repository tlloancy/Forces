...
function feuille_ordre1 (depart : int, arriver : int, piece) {
	PlayerPrefs.SetInt("Orde" + ordre + "departia1", depart);
	PlayerPrefs.SetInt("Orde" + ordre + "arriveria1", arriver);
	PlayerPrefs.SetString("Orde" + ordre + "pieceia1", piece);
	ordre++;
}
	...
	if (depart.IndexOf("change") != -1) {
		...
		for (var dec : GameObject in decor) {
			if (dec.name == arriver) {
				...
				GameObject.Find("ScriptSF").GetComponent(AnimateGnrl).fight(true, dec, -3);
			}
		}
	}
	else {
			...
			if (dec.name == depart) {
				for (var mov : GameObject in decor) {
					if (mov.name == arriver) {
						...
						GameObject.Find("ScriptSF").GetComponent(AnimateGnrl).fight(true, mov, -3);
					}
				}
			}
		}
...
function exe_ordre2 (piece, arrivee) {
	...
	for (var dec : GameObject in decor) {
		if (dec.name == arrivee) {
			...
			GameObject.Find("ScriptSF").GetComponent(AnimateGnrl).change(piece, dec, scriptinfo1); // Color.red); variable script pansement pour anti-réseau voir AnimateGnrl
		}
	}
}
...
function exe_all_ordre () {
	...
	wait_exe = 0;
	while (++nbordre < ordre) {
		var recup1 = PlayerPrefs.GetInt("Orde" + nbordre + "departia1");
		var recup2 = PlayerPrefs.GetInt("Orde" + nbordre + "arriveria1");
		var recup3 = PlayerPrefs.GetString("Orde" + nbordre + "pieceia1");
		wait_ordre = 0;
		if (recup1 < 57 && hbombready == 0)
			exe_ordre1(scripttest.biblidecor1[recup1], scripttest.biblidecor1[recup2], recup3);
		else
		{
			if (recup1 == 57 && recup2 != -1 && hbombready == 0)
				exe_ordre2(recup3, scripttest.biblidecor1[recup2]);
			else if (hbombready && recup1 != 57){
				exe_ordre_h_bomb(scripttest.biblidecor1[recup1], scripttest.biblidecor1[recup2], recup3, scriptinfo1);
				Debug.LogWarning("ben ques que tu fais");//
				Debug.LogWarning(scripttest.biblidecor1[recup1]);//
				Debug.LogWarning(scripttest.biblidecor1[recup2]);//
				Debug.LogWarning(recup3);//
				hbombready = 0;
			}
			else if (hbombready && recup1 == 57){
				exe_ordre_h_bomb("changedec", scripttest.biblidecor1[recup2], recup3, scriptinfo1);
				Debug.LogWarning(scripttest.biblidecor1[recup2]);//
...
function recup_ordre () 
{
	if (Application.loadedLevelName == "Battleground")
	{
		...
		scriptinfo1 = GameObject.Find(propre("red")).GetComponent(allInfo); // IA de reference ordre blue red yellow green
		scriptinfo2 = GameObject.Find(propre("green")).GetComponent(allInfo);// Donc scriptinfo 1 -> script de l ia 
		scriptinfo3 = GameObject.Find(propre("yellow")).GetComponent(allInfo);
		scriptinfo4 = GameObject.Find(propre("blue")).GetComponent(allInfo); // Script info 1 zone 0 etc a optimiser plus tard
		if (scriptchoose.round1 == 1 && scriptchoose.ia == scriptchoose.round1) // est-ce le premier round
		{
			var coords = 0;
			var onehunterhome = 0;
		...
		if (scriptchoose.round1 != 1 && scriptchoose.ia == scriptchoose.round1) // est-ce le premier round
		{
			...
			{
						for (var dec : GameObject in decor) {
							if (dec.name == scripttest.biblidecor1[count])
	...
	//		 random = Random.Range(0, 11);
			...
			*/
			...
			coords = 0;//Truc supermassif mais en fait : 3 modes, et 3 ia dans chaque mode dont le seul changement est le tableau biblidecor1 utilise
			random = 0;
